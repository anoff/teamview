...
module.exports = class Flight {
  constructor (r = {}, submittingToken) {
    ...
    if (!this.fromLocation && this.fromGalaxy && this.fromSystem && this.fromPosition) {
      this.fromLocation = `${r.fromGalaxy.toString().padStart(3, '0')}:${r.fromSystem.toString().padStart(3, '0')}:${r.fromPosition.toString().padStart(3, '0')}`
    }
    if (!this.toLocation && this.toGalaxy && this.toSystem && this.toPosition) {
      this.toLocation = `${r.toGalaxy.toString().padStart(3, '0')}:${r.toSystem.toString().padStart(3, '0')}:${r.toPosition.toString().padStart(3, '0')}`
    }
  }
