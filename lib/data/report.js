...
module.exports = class Report {
   ...
   * @param {string} bodyType specify if searching for 'planet' or 'moon' or 'either', default: planet
   * @returns Array[Report]
   */
  static async getByLocation (galaxy, system, position = 0, type = 'latest', bodyType = 'planet') {
    ...
    if (bodyType === 'moon') {
      ...
      search.is_moon = false
    } else if (bodyType === 'either') {
      // do nothing
    }
  ...
  static async search (query, senderTokenId = -1) {
    const cmd = knex
      .select(knex.raw('reports.*, EXTRACT(EPOCH FROM now() - reports.date)/3600 as report_age_hrs, planets.planet_name, players.player_name, players.alliance, players.rank, players.is_inactive, players.is_vacation, players.is_banned, planets.player_id'))
      .from('reports')
      .leftJoin('planets', 'reports.location', 'planets.location')
      .leftJoin('players', 'players.player_id', 'planets.player_id')
      .whereNotNull('players.player_id')
      .where(whereF => {
        whereF.where('reports.galaxy', '>', 0)
        whereF.where('reports.system', '>', 0)
        whereF.where('reports.position', '>', 0)
        if (query.report_type === undefined) {
          whereF.where('reports.report_type', '=', 'espionage')
        } else {
          whereF.where('reports.report_type', '=', query.report_type)
        }
        if (query.is_moon === 'true') whereF.where('reports.is_moon', '=', true)
        if (query.is_moon === 'false') whereF.where('reports.is_moon', '=', false)
        ...
        // if system_min > system_max we overflow to the beginging of a system and
        // need to adjust the query
        let addedSystemCondition = false
        if (query.system_min && query.system_max) {
          if (query.system_min > query.system_max) {
            whereF.where(subquery => {
              subquery
                .where('reports.system', '>=', parseInt(query.system_min))
                .orWhere('reports.system', '<=', parseInt(query.system_max))
            })

            addedSystemCondition = true
          }
        }

        // default behaviour of getting reports in range
        if (!addedSystemCondition) {
        ...
        if (parseInt(query.rank_min)) whereF.where('rank', '>=', parseInt(query.rank_min))
        if (parseInt(query.rank_max)) whereF.where('rank', '<=', parseInt(query.rank_max))
        if (parseInt(query.min_crystal)) whereF.where(knex.raw(`(reports.resources->>'crystal')::int >= ${parseInt(query.min_crystal) * 1e3}`))
        if (parseInt(query.min_deuterium)) whereF.where(knex.raw(`(reports.resources->>'deuterium')::int >= ${parseInt(query.min_deuterium) * 1e3}`))
        if (parseInt(query.min_mse)) whereF.where(knex.raw(`((resources->>'metal')::int + (resources->>'crystal')::int *4 + (resources->>'deuterium')::int* 4) >= ${parseInt(query.min_mse) * 1e3}`))
        if (parseInt(query.max_tech)) whereF.where(knex.raw(`GREATEST((research->>'weaponsTechnology')::int,(research->>'shieldTechnology')::int,(research->>'armourTechnology')::int) <= ${parseInt(query.max_tech)}`))
        ...
        if (query.report_maxage) whereF.where(knex.raw(`EXTRACT(EPOCH FROM now() - reports.date)/3600 <= ${parseFloat(query.report_maxage)}`))
      })
  ...
  static async getByReportId (reportId) {
    ...
    return Report.fromDB(response[0])
  }
}
