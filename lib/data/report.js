...
module.exports = class Report {
  ...
  async sync () {
    ...
    if (this.planetId < 0) {
      ...
      if (another) {
        this.planetId = another.planetId
      }
    }
  ...
  static async getByLocation (galaxy, system, position = 0, type = 'latest') {
    ...
    switch (type.toLocaleLowerCase()) {
      ...
      case 'latest':
        select = knex.raw('distinct on (CONCAT(reports.galaxy, \':\', reports.system, \':\', reports.position)) date, resources, buildings, ships, research, defense, reports.galaxy, reports.system, reports.position, report_id')
        break
      case 'all':
      default:
  ...
  static async search (query) {
    const cmd = knex
      .select(knex.raw('reports.*, EXTRACT(EPOCH FROM now() - reports.created_at)/3600 as report_age_hrs, planets.planet_name, players.player_name, players.alliance, players.rank, players.is_inactive, players.is_vacation, players.is_banned'))
      .from('reports')
      .leftJoin('planets', 'planets.planet_id', 'reports.planet_id')
      ...
      .where(whereF => {
        ...
        if (query.report_maxage) whereF.where(knex.raw(`EXTRACT(EPOCH FROM now() - reports.created_at)/3600 <= ${parseInt(query.report_maxage)}`))
      })
      .orderBy('id', 'desc')
