...
module.exports = class Report {
  ...
  static async search (query, senderTokenId = -1) {
    ...
    */
    const cmd = knex
      .select(knex.raw('reports.*, EXTRACT(EPOCH FROM now() - reports.date)/3600 as report_age_hrs, planets.planet_name, players.player_name, players.alliance, players.rank, players.is_inactive, players.is_vacation, planets.player_id'))// , COALESCE(t.allships, 0) allships'))
      .from('reports')
      // .leftJoin(knex.raw(`
      //   (SELECT r.id, SUM(l.value::int) allships
      //   FROM reports r
