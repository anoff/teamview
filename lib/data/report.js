...
module.exports = class Report {
  ...
  static async search (query, senderTokenId = -1) {
    const cmd = knex
      .select(knex.raw('reports.*, EXTRACT(EPOCH FROM now() - reports.date)/3600 as report_age_hrs, planets.planet_name, players.player_name, players.alliance, players.rank, players.is_inactive, players.is_vacation, players.is_banned, planets.player_id, flights.date as last_attack, flights.from_location as attack_from_location'))
      .from('reports')
      ...
      .leftJoin(knex.raw(`(
          ...
          ON reports.location = flights.to_location`))
      .where(whereF => {
    ...
    return response.map(e => {
      ...
      if (e.lastAttack) {
        data.lastAttack = e.lastAttack
        data.attackFromLocation = e.attackFromLocation
      }
