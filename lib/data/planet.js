...
module.exports = class Planet {
  ...
  static async search (query) {
    const cmd = knex
      .select(knex.raw("DISTINCT on (planets.location) '' as flights_data, flights.*, flights.date as last_attack, '' as reports_data, reports.*, '' as planets_data, planets.*, '' as players_data, players.*, EXTRACT(EPOCH FROM now() - reports.created_at)/3600 as report_age_hrs")) // put planets last so they overwrite (potentially) missing info from reports
      .from('planets')
    ...
    return response.map(e => {
      ...
      const f = Flight.fromDB(e)
      if (f.mission) {
        console.log({ e, f })
        f.date = e.lastAttack
        p.extras.flight = f
      }
      p.extras.lastAttack = e.lastAttack || null
      return p
    })
  }
}
