...
module.exports = class Planet {
  ...
  static async search (query) {
    const cmd = knex
      .select(knex.raw('distinct on (CONCAT(planets.galaxy, \':\', planets.system, \':\', planets.position)) *, planets.galaxy, planets.system, planets.position, EXTRACT(EPOCH FROM now() - reports.created_at)/3600 as report_age_hrs'))
      .from('planets')
      ...
      .where(whereF => {
        ...
        // if (query.report_maxage) whereF.where('reports.report_id', '>', 0).where('report_age_hrs', '<=', parseInt(query.report_maxage))
        // TODO: causes error in case no report exists column does not exist
      })
      .limit(100)
    console.log()
