...
module.exports = class Planet {
  ...
  static async search (query) {
    const cmd = knex
      .select(knex.raw("DISTINCT on (planets.location) '' as flights_data, flights.*, '' as reports_data, reports.*, '' as planets_data, planets.*, '' as players_data, players.*, EXTRACT(EPOCH FROM now() - reports.created_at)/3600 as report_age_hrs, flights.date as flight_date")) // put planets last so they overwrite (potentially) missing info from reports
      .from('planets')
