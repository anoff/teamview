...
module.exports = class Planet {
  ...
  static async search (query) {
    const cmd = knex
      .select(knex.raw(`distinct on (location) *, 
      CONCAT(planets.galaxy, ':', planets.system, ':', planets.position) location,
      planets.galaxy, planets.system, planets.position, planets.planet_id,
      EXTRACT(EPOCH FROM now() - reports.created_at)/3600 as report_age_hrs, planets.updated_at,
      planets.moon_id`)) // fetch moon_id again from planets because otherwise might get overwritten by reports.moon_id for a planet
      .from('planets')
      .leftJoin('players', 'planets.player_id', 'players.player_id')
      .leftJoin('reports', 'reports.planet_id', 'planets.planet_id')
      .where(whereF => {
        whereF.where('planets.galaxy', '>', 0)
        whereF.where('planets.system', '>', 0)
        whereF.where('planets.position', '>', 0)
