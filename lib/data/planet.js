...
module.exports = class Planet {
   ...
   * @param {object} query
   * @returns Array[Planet] including .report data
   */
  static async search (query) {
    const cmd = knex
      .select(knex.raw("DISTINCT on (planets.location) '' as reports_data, reports.*, '' as planets_data, planets.*, '' as players_data, players.*, EXTRACT(EPOCH FROM now() - reports.created_at)/3600 as report_age_hrs")) // put planets last so they overwrite (potentially) missing info from reports
      .from('planets')
      ...
      .where(whereF => {
        ...
        try {
          if (query.system_min && query.system_max && (parseInt(query.system_min) > parseInt(query.system_max))) {
            whereF.where(subquery => {
              subquery
                .where('planets.system', '>=', parseInt(query.system_min))
                .orWhere('planets.system', '<=', parseInt(query.system_max))
            })
          } else {
        ...
        if (query.require_report === 'true' && query.report_maxage) whereF.where(knex.raw(`EXTRACT(EPOCH FROM now() - reports.created_at)/3600 <= ${parseFloat(query.report_maxage)}`))
      })
      .orderBy('planets.location', 'asc')
