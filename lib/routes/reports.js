 ...
 */
router.post('/v1/reports', body('reports').isArray(), header('token').isString(), validateRequest, checkToken, async (req, res) => {
...
/**
 * @api {get} /v1/reports/:reportId Get report by ID
 * @apiName GetReportById
 * @apiGroup Report
 * @apiDescription Retrieve a single report by report ID
 *
 * @apiHeader {string} content-type='application/json; charset=utf-8' Encode as JSON
 * @apiHeader {string} token A valid API token (starting with `TOKEN_`)
 * @apiParam {int} reportId report ID as found in the teamview report search
 *
 * @apiSuccess {Report} report Report matching the given report ID
 *
 * @apiError Unauthorized The provided token in the header is invalid
 * @apiError InternalServerError Something went wrong. See the error text in the json response
 */
router.get('/v1/reports/:reportId', header('token').isString(), validateRequest, checkToken, async (req, res) => {
  const reportId = req.params.reportId

  const r = await Report.getByReportId(reportId)
  if (req.query.extended) {
    const p = await Planet.getByPlanetId(r.planetId)
    if (p.id > 0) {
      r.extras = {}
      r.extras.planet = p
      const py = await Player.getById(p.playerId)
      if (py.id) {
        r.extras.player = py
      }
    }
