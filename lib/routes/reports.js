...
router.get('/v1/reports/', header('token').isString(), async (req, res) => {
  ...
  if (req.query.type === 'mine') {
    // return submitted report ids by me
    try {
      const t = await Token.getByValue(token)
      const res = await Report.getReportIdsByTokenId(t.id)
      logger.info({ token }, 'Fetched existing reports')
      res.json(res)
    } catch (e) {
      logger.error(e, 'Failed to fetch existing reports')
      return res.json(500, { message: 'Failed to fetch existing reports', error: e })
    }
  }
})
