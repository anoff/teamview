...
*/
router.post('/v1/planets', body('planets').isArray(), header('token').isString(), validateRequest, checkToken, async (req, res) => {
 ...
 * @apiError InternalServerError Something went wrong. See the error text in the json response
 */
router.get('/v1/planets/:location', header('token').isString(), validateRequest, checkToken, async (req, res) => {
  ...
  const p = await Promise.all(location.split(',').map(async l => {
    ...
    if (req.query.type === 'report') {
      const [planets, reports] = await Promise.all([await Planet.getByLocation(g, s, pos, type), await Report.getByLocation(g, s, pos, 'latest', 'planet')])
 ...
 * @apiError InternalServerError Something went wrong. See the error text in the json response
 */
router.get('/v1/planets', header('token').isString(), validateRequest, checkToken, async (req, res) => {
 ...
 * @apiHeader {string} content-type='application/json; charset=utf-8' Encode as JSON
 * @apiHeader {string} token A valid API token (starting with `TOKEN_`)
 * @apiParam {string} location The location of the planet in format GALAXY:SYSTEM:POSITION
 *
 * @apiError Unauthorized The provided token in the header is invalid
 * @apiError InternalServerError Something went wrong. See the error text in the json response
 */
router.delete('/v1/planets/:location', header('token').isString(), validateRequest, checkToken, async (req, res) => {
  const location = req.params.location
