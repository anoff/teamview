 ...
 * @apiBody {int} planets.id Ingame ID of the planet
 * @apiBody {string} planets.name The name of the planet
 ...
 * @apiBody {string[]} planets.playerStatus A list of status attributes (longinactive, inactive, banned, vacation)
...
router.post('/v1/planets', body('planets').isArray(), header('token').isString(), async (req, res) => {
  ...
  for (const planetData of planets) {
    ...
    planetData.ingameId = planetData.id
    delete planetData.id

    let planet = await Planet.getByIngameId(planetData.ingameId)
    if (!planet?.id) {
...
/**
 * @api {get} /v1/plants Search for planets
...
router.get('/v1/planets', header('token').isString(), async (req, res) => {
  ...
  const query = { }
  for (const k of ['galaxyMin', 'galaxyMax', 'systemMin', 'systemMax', 'rankMin', 'rankMax', 'playerName', 'allianceName']) {
    if (req.query[k]) {
      query[k] = req.query[k]
    }
  }
  const response = await Planet.search(query)
  res.json(response)
})
