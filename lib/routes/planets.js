...
router.get('/v1/planets', header('token').isString(), async (req, res) => {
  ...
  const query = { }
  for (const k of ['galaxy_min', 'galaxy_max', 'system_min', 'system_max', 'rank_min', 'rank_max', 'player_name', 'alliance_name', 'require_report', 'vacation', 'banned', 'inactive', 'report_maxage']) {
    if (req.query[k]) {
      query[k] = req.query[k]
    }
  }
