...
router.post('/v1/planets', ...middlewares, async (req, res) => {
  ...
  for (const planetData of planets) {
    ...
    const [player, planets] = await Promise.all([await Player.getByName(playerName), await Planet.getByLocation(planetData.galaxy, planetData.system, planetData.position)])
    if (player.ingameId > -1) {
      planetData.playersIngameId = player.ingameId
...
router.get('/v1/planets/:location', header('token').isString(), async (req, res) => {
  ...
  const p = await Promise.all(location.split(',').map(async l => {
    ...
    const [planet, report] = await Promise.all([Planet.getByLocation(g, s, pos, type), Report.getByLocation(g, s, pos, 'latest')])
    Object.assign(planet, report.flat())
    return planet
  }))
  res.json(p.flat())
})
