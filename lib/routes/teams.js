...
router.post('/v1/teams', body('name').isString(), body('token').isString(), async (req, res) => {
...
router.post('/v1/teams/join', ...middlewares2, async (req, res) => {
  ...
  const token = req.body.token
  const tok = Token.getByValue(token)
  if (tok == null || tok.password !== Token.hashPassword(req.headers.password)) {
